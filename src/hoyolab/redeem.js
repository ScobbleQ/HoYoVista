...
export const redeemCode = async (id, { arrayOfGameId, hoyolabCookies, linkedGames, isPrivate, toNotify, automatic }) => {
	...
	await Promise.all(
		arrayOfGameId.map(async (gameId) => {
			...
			const unredeemedCodes = availableCodes[gameKey]?.filter(code => !attempted_codes.includes(code.code)) || [];
