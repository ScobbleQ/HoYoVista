...
import { WEB_HEADER, CHECKIN_URL, CHECKIN_DETAILS_URL, GAME_ICON_URL } from '../utils/routes.js';
...
const createEmbed = ({ nickname, uid, gameId, color, title, description, thumbnail, footer }) => {
	const embed = new EmbedBuilder()
		.setColor(color)
		.setAuthor({ name: `${nickname} (${uid})`, iconURL: GAME_ICON_URL[gameId] })
		.setTitle(title)
...
export const performCheckin = async ({ arrayOfGameId, linkedGames, hoyolabCookies, isPrivate, to_notify_checkin, automatic }) => {
	...
	for (const gameId of arrayOfGameId) {
		const { game_role_id, nickname, auto_checkin } = Object.values(linkedGames).find(game => game.game_id === gameId);
