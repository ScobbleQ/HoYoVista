...
export default {
	...
	renderGeneralSettings(data, page) {
		const descriptions = {
			updateNotifications: Description.updateNotifications.replace('{status}', data.settings.subscribed ? 'Enabled' : 'Disabled'),
		...
		const embed = new EmbedBuilder()
			...
			.setDescription(`${descriptions.updateNotifications}\n\n${descriptions.analytics}\n\n${descriptions.privacyBlur}`);
		...
		return { embeds: [embed], components: [menu, buttons], ephemeral: data.settings.is_private };
	},
	...
	renderResetSettings() {
		const embed = createEmbed('Are you sure you want to reset your settings?\n\nThis action cannot be reversed.', embedColors.warning);

		const confirmButton = new ButtonBuilder().setCustomId('settings-reset-confirm').setLabel('Confirm').setStyle(ButtonStyle.Danger);
		const cancelButton = new ButtonBuilder().setCustomId('settings-reset-cancel').setLabel('Cancel').setStyle(ButtonStyle.Secondary);
	...
	async handleButtonClick(interaction) {
		...
		case Actions.SUBSCRIBE:
			await mongo.set(interaction.user.id, { field: 'settings.subscribed', value: curValue === 'true' ? false : true });
			await this.execute(interaction, true, 1);
			break;
		case Actions.DATA:
			await mongo.set(interaction.user.id, { field: 'settings.collect_data', value: curValue === 'true' ? false : true });
			await this.execute(interaction, true, 1);
			break;
		case Actions.PRIVACY:
			await mongo.set(interaction.user.id, { field: 'settings.is_private', value: curValue === 'true' ? false : true });
			await this.execute(interaction, true, 1);
			break;
		case Actions.CHECKIN:
			await mongo.set(interaction.user.id, { field: `linked_games.${IdToAbbr[id]}.${action}`, value: curValue === 'true' ? false : true });
