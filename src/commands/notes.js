...
export default {
	...
	async execute(interaction) {
		...
		const gameKey = Object.keys(linkedGames).find(
			key => linkedGames[key].game_id === gameId,
		);
		...
		const notesEmbed = createEmbed(`Notes retrieved in ${noteFetchTime}ms.\nPreparing your data...`, embedColors.warning);
		await interaction.editReply({ embeds: [notesEmbed] });

		const embeds = [];
		if (gameId === Game.GENSHIN) {
			const resinLevel = `${notes.current_resin}/${notes.max_resin}`;
			const resinRestore = notes.resin_recovery_time === '0' ? 'Fully Replenished' : `Full <t:${Math.floor(Date.now() / 1000) + Number(notes.resin_recovery_time)}:R>`;
			const base = new EmbedBuilder()
				.setColor(embedColors.primary)
				.setAuthor({ name: `${nickname} (${game_role_id})`, iconURL: GAME_ICON_URL[gameId] })
				.setDescription(`**Original Resin** ${resinLevel}\n${resinRestore}`);

			if (notes.max_home_coin !== '0') {
				const realmMax = notes.home_coin_recovery_time === '0' ? 'Already Full' : `Replenishes <t:${Math.floor(Date.now() / 1000) + Number(notes.home_coin_recovery_time)}:R>`;
				base.addFields({ name: 'Jar of Riches', value: `Realm Currency ${notes.current_home_coin}/${notes.max_home_coin}\n${realmMax}` });
			}

			if (notes.resin_discount_num_limit) {
				base.addFields({ name: 'Enemies of Note', value: `Discount ${notes.remain_resin_discount_num}/${notes.resin_discount_num_limit}` });
			}

			if (notes.max_expedition_num) {
				base.addFields({ name: 'Expedition', value: `Dispatched ${notes.current_expedition_num}/${notes.max_expedition_num}` });
			}
			...
			embeds.push(new EmbedBuilder()
				...
				.addFields(
					{ name: 'Daily Commissions', value: `${notes.daily_task.finished_num}/${notes.daily_task.total_num}` },
					{ name: 'Long-Term Encounter Points', value: `x${notes.daily_task.stored_attendance}` },
				));
		}
		else if (gameId === Game.ZZZ) {
			const batteryLevel = `${notes.energy.progress.current}/${notes.energy.progress.max}`;
			const batteryRestore = notes.energy.restore === 0 ? 'Fully Recovered' : `Full <t:${Math.floor(Date.now() / 1000) + notes.energy.restore}:R>`;
			embeds.push(new EmbedBuilder()
				.setColor(embedColors.primary)
				.setAuthor({ name: `${nickname} (${game_role_id})`, iconURL: GAME_ICON_URL[gameId] })
			...
			embeds.push(new EmbedBuilder()
				...
				.addFields(
					{ name: 'Engagement Today', value: `${notes.vitality.current}/${notes.vitality.max}` },
					{ name: 'Scratch Card Mania', value: notes.card_sign === 'CardSignNo' ? 'Incomplete' : 'Complete' },
					{ name: 'Video Store Management', value: notes.vhs_sale.sale_state === 'SaleStateDone' ? 'Revenue Available' : 'Currently Open' },
				));
			...
			const riduPoints = notes.weekly_task ? `${notes.weekly_task.cur_point}/${notes.weekly_task.max_point}` : '-';
			const riduReset = notes.weekly_task ? `Refreshes <t:${Math.floor(Date.now() / 1000) + notes.weekly_task.refresh_time}:R>` : '-';
			embeds.push(new EmbedBuilder()
				.setColor(embedColors.primary)
				.setTitle('Season Missions')
				.addFields(
					{ name: 'Bounty Commission', value: `Progress ${bountyProgress}\n${bountyReset}` },
